...
namespace GreeenGarden.Business.Service.TakecareComboOrderService
{
    public class TakecareComboOrderService : ITakecareComboOrderService
    {
        ...
        public async Task<ResultModel> GetServiceOrderDetailByRangeDate(OrderRangeDateReqModel model, PaginationRequestModel pagingModel, string token)
        {
            if (!string.IsNullOrEmpty(token))
            {
                string userRole = _decodeToken.Decode(token, ClaimsIdentity.DefaultRoleClaimType);
            ...
            try
            {
                Page<TblTakecareComboOrder> listTblServiceOrders = await _takecareComboServiceRepo.GetAllServiceOrderByRangDate(pagingModel, fromDate, toDate);
                List<ServiceOrderGetResModel> resList = new();
                if (listTblServiceOrders != null)
                {
                    List<TakecareComboOrderModel> takecareComboOrderModelList = new();
                    foreach (var item in listTblServiceOrders.Results)
                    {
                        TakecareComboOrderModel takecareComboOrderModelAdd = await GetTakecareComboOrder(item.Id);
                        takecareComboOrderModelList.Add(takecareComboOrderModelAdd);
                    }
